...
if ($_POST) {
    ...
    // Validation
    if (empty($username) || empty($email) || empty($password) || empty($firstName) || empty($lastName) || empty($role) || empty($schoolId)) {
        $error = 'Please fill in all required fields.';
    } elseif (!in_array($role, ['student', 'teacher'])) {
        $error = 'Please select a valid role.';
    } elseif ($role === 'student' && empty($studentId)) {
        $error = 'Student ID is required for student accounts.';
    } elseif ($role === 'teacher' && empty($teacherId)) {
        $error = 'Teacher ID is required for teacher accounts.';
        ...
        $error = 'Please upload your ID photo.';
    } else {
        ...
        if ($existingUser) {
            ...
            if (!$school) {
                ...
                if ($userId) {
                    logActivity($db, $userId, $role, 'register', "New $role account created");
                    ...
                    $redirectUrl = $role === 'teacher' ? '../teacher/dashboard.php' : '../student/dashboard.php';
                    redirect($redirectUrl, 'Welcome to the Capstone System, ' . $firstName . '!', 'success');
                } else {
    ...
    <div class="auth-card" style="max-width: 550px;">
        ...
        <form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
            ...
            <div id="teacherFields" style="display: none;">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="teacher_id" name="teacher_id" 
                           value="<?php echo htmlspecialchars($teacherId ?? ''); ?>" 
            ...
            <div class="form-group">
                ...
                <div class="form-text">Upload a clear photo of your ID (JPG, JPEG, or PNG format, max 5MB)</div>
                <div class="invalid-feedback">Please upload your ID photo.</div>
            </div>
